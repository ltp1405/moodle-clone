#include "Greeting.h"
#include <iostream>
using namespace std;

void loginGreeting() {
    cout << " _                _       "   << endl;
    cout << "| |    ___   __ _(_)_ __  "   << endl;
    cout << "| |   / _ \\ / _` | | '_ \\ " << endl;
    cout << "| |__| (_) | (_| | | | | |"   << endl;
    cout << "|_____\\___/ \\__, |_|_| |_|" << endl;
    cout << "            |___/         "   << endl;
}

void welcomeGreeting() {
    cout << "__        __   _                           " << endl;
    cout << "\\ \\      / /__| | ___ ___  _ __ ___   ___  " << endl;
    cout << " \\ \\ /\\ / / _ \\ |/ __/ _ \\| '_ ` _ \\ / _ \\ " << endl;
    cout << "  \\ V  V /  __/ | (_| (_) | | | | | |  __/ " << endl;
    cout << "   \\_/\\_/ \\___|_|\\___\\___/|_| |_| |_|\\___| " << endl;
    cout << "                                           " << endl;
}

void schoolyearGreeting() {
    cout << "   ____    __             __                   " << endl;
    cout << "  / __/___/ /  ___  ___  / /_ _____ ___ _____  " << endl;
    cout << " _\\ \\/ __/ _ \\/ _ \\/ _ \\/ / // / -_) _ `/ __/  " << endl;
    cout << "/___/\\__/_//_/\\___/\\___/_/\\_, /\\__/\\_,_/_/     " << endl;
    cout << "                         /___/                 " << endl;
}

void profileGreeting() {
    cout << "   ___           ____ __     " << endl;
    cout << "  / _ \\_______  / _(_) /__   " << endl;
    cout << " / ___/ __/ _ \\/ _/ / / -_)  " << endl;
    cout << "/_/  /_/  \\___/_//_/_/\\__/   " << endl;
    cout << "                             " << endl;
}

void semesterGreeting() {
    cout << "   ____                  __           " << endl;
    cout << "  / __/__ __ _  ___ ___ / /____ ____  " << endl;
    cout << " _\\ \\/ -_)  ' \\/ -_|_-</ __/ -_) __/  " << endl;
    cout << "/___/\\__/_/_/_/\\__/___/\\__/\\__/_/     " << endl;
    cout << "                                      " << endl;
}

void courseGreeting() {
    cout << "  _____                       " << endl;
    cout << " / ___/__  __ _________ ___   " << endl;
    cout << "/ /__/ _ \\/ // / __(_-</ -_)  " << endl;
    cout << "\\___/\\___/\\_,_/_/ /___/\\__/   " << endl;
    cout << "                              " << endl;
}

void scoreboardGreeting() {
    cout << "   ____                 __                    __ " << endl;
    cout << "  / __/______  _______ / /  ___  ___ ________/ / " << endl;
    cout << " _\\ \\/ __/ _ \\/ __/ -_) _ \\/ _ \\/ _ `/ __/ _  /  " << endl;
    cout << "/___/\\__/\\___/_/  \\__/_.__/\\___/\\_,_/_/  \\_,_/   " << endl;
    cout << "                                                 " << endl;
}
